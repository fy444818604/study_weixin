...
Page({
  ...
  data: {
    ...
    orderShow: [
      { "title": "学分从低到高", "key": "ASCENDING" },
      { "title": "学分从高到低", "key": "DESCENDING" }
    ],
    memberTypeList: [],//会员列表
  ...
  onLoad: function () {
    ...
    api.websiteProfileInfo({
      ...
      success: function (res) {
        ...
        if (that.data.serviceSwitch.PackageSwitch != 'ON') {
          that.setData({
            sellType: "COURSE"
          });
        }
  ...
  openCourse: function (event) {
    ...
    wx.navigateTo({
      url: '/pages/web/exam/examDetail/examDetail?id=' + courseId,
    })
  }
})